...
object Main
{
    private fun getIssueAtPage(page: Int) = URL("https://api.github.com/repos/996icu/996.ICU/issues?state=all&filter=all&page=$page&access_token=${Token.ACCESS_TOKEN}").openConnection().getInputStream().bufferedReader().readText()
...
//                "${obj["created_at"]}\t${obj["number"]}\t${(obj["user"] as JSONObject)["login"]}\t${(obj["user"] as JSONObject)["id"]}\t${obj["title"]}\t${(obj["body"] as String).replace("\r", "").replace("\n", "\\n").replace("\t", "\\t")}"
    ...
    {
//        getAllIssue()
        analyseAllIssue().groupBy { it.user_login }.map { it.key to it.value.size }.sortedByDescending { it.second }.forEach { println("|${it.first}|${it.second}|") }
